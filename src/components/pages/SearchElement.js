      ...
      console.error(err);
        ...
        const clearInput = () => {
          ...
          const displaySearchedTrucks =  filteredDataTrucks.slice(0, 15).map((value , key)=>{
            return(
              <TouchableOpacity  style={{flex : 1, marginBottom :6 , padding : 6}} key={value.id} onPress={()=> navigate(`selectedUserTrucks/${value.userId}/${value.timeStamp}/${value.CompanyName}} ` ) }>
        ...
        const displaySearched =   filteredData.slice(0, 15).map((value , key)=>{
            return(
            <TouchableOpacity  style={{flex : 1, marginBottom :6 , padding : 6}} key={value.id} onPress={()=> navigate(`searchedLoads/${value.userId}/${value.timeStamp}/${value.CompanyName}` ) }>
